...
exports.pck2wem = async ({ pckFile, processingDir }) => {
  ...
  const quickBMS =
    platform === 'win32'
      ? path.join('.', 'libs', 'quickbms', 'quickbms.exe')
      : path.join('.', 'libs', 'quickbms', 'quickbms');
  const script =
    b || bmsScript
      ? path.join('.', 'libs', 'quickbms', 'scripts', b || bmsScript)
      : path.join('.', 'libs', 'quickbms', 'scripts', 'wavescan.bms');
